...
TEXT_DIM_COLS = ["COUNTRY", "CITY", "ZONE", "METRIC"]


def clean_orders(excel_path: Path, output_dir: Path) -> Dict[str, object]:
    raw_df = load_sheet(excel_path, ORDERS_SHEET)
    source_columns = list(raw_df.columns)

    profile: Dict[str, object] = {
        "source_table": ORDERS_SHEET,
    ...
    df["SOURCE_TABLE"] = ORDERS_SHEET

    ordered_cols: List[str] = ["COUNTRY", "CITY", "ZONE", "METRIC", "SOURCE_TABLE", "_SOURCE_ROW_NUMBER"]
    ordered_cols += [f"{c}_ORIGINAL" for c in TEXT_DIM_COLS if f"{c}_ORIGINAL" in df.columns]

    original_week_cols = [c for c in df.columns if c.startswith("L") and c.endswith("W") and c not in present_week_cols]
    ...
    safe_write(
        cleaned,
        output_dir / "orders_raw_cleaned.csv",
        output_dir / "orders_raw_cleaned.parquet",
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Clean RAW_ORDERS sheet.")
    ...
    profile = clean_orders(excel_path, paths.processed_dir)
    print(profile)
